...
class Jak3MissionData:
    ...
    def __init__(self, mission_id: int, task_id: int, name: str, rule: Optional[Callable] = None,
        ...
        self.name = name
        self.has_rule = rule is not None
        if rule:
            self.rule = rule
        else:
...
main_mission_table = {
    ...
    2: Jak3MissionData(mission_id=2, task_id=11, name="Earn 1st war amulet",
                       items_granted=["Dark Jak", "Blaster", "War Amulet #1", "Gate Pass to Spargus"]),
...
side_mission_table = {
    ...
    154: Jak3SideMissionData(mission_id=154, task_id=131, name="Desert Time Trial",
                             rule=lambda state, player:
                             spargus_to_desert(state, player)),
    155: Jak3SideMissionData(mission_id=155, task_id=132, name="Desert Rally Side Mission",
                             rule=lambda state, player:
                             spargus_to_desert(state, player)),
...
def get_dual_check_mission_locations() -> dict[str, int]:
    locations = {}
    for mission_id, mission in main_mission_table.items():
        locations[mission.name] = get_location_id(mission_id, DUAL_CHECK_SLOT_OFFSET)
